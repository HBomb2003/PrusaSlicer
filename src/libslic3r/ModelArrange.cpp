        ...
        if (! objptr->instances.empty()) {
            ...
            // x, y rotation that is why we use the first instance.
            {
                ...
                if(tolerance > EPSILON) {
                    Polygons pp { p };
                    pp = p.simplify(double(scale_(tolerance)));
                 ...
                 WipeTowerInfo &               wti)
{
    for(auto& r : group) {
        ...
        ModelInstance *inst_ptr = shapemap[idx].first;
            ...
            Vec3d foff(unscale(off.X + batch_offset) ,
                       unscale(off.Y),
    ...
    if(!preshapes.front().empty()) { // If there is something on the plate
        ...
        for(auto shit = shapes.begin(); shit != shapes.end(); ++shit, ++shptrit)
        {
            ...
            if(!arranger.is_colliding(itm)) {
                ...
                Vec3d foffset(unscale(offset.X),
                              unscale(offset.Y),
                              minst->get_offset()(Z));
             ...
             std::function<bool ()> stopcondition)
{
    ...
    coord_t md = ceil_i(int(min_obj_distance), int(2)) - SCALED_EPSILON;
